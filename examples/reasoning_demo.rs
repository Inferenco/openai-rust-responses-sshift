...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("📊 Response Status: {status}", status=math_response.status);
    ...
    println!("�� Response Status: {status}", status=logic_response.status);
    ...
    println!("📊 Final Response Status: {status}", status=creative_response.status);
