...
impl Error {
    ...
    pub fn is_recoverable(&self) -> bool {
        match self {
            // Container expiration is always recoverable
            Self::ContainerExpired { .. } => true,
            
            // Server errors that are always recoverable
            Self::BadGateway { .. } 
            | Self::ServiceUnavailable { .. } 
            | Self::GatewayTimeout { .. } => true,
            
            // Server errors that may be recoverable
            Self::ServerError { retry_suggested, .. } => *retry_suggested,
    ...
    pub fn retry_after(&self) -> Option<u64> {
        match self {
            ...
            Self::ServerError { retry_suggested: true, .. } => Some(5), // 5 seconds
    ...
    pub fn user_message(&self) -> String {
        match self {
            ...
            Self::AuthenticationFailed { suggestion, .. } => suggestion.clone(),
            Self::AuthorizationFailed { suggestion, .. } => suggestion.clone(),
            
            Self::ClientError { suggestion: Some(suggestion), .. } => suggestion.clone(),
            ...
            Self::InvalidApiKey => {
                "Invalid API key. Please check your API key format.".to_string()
            }
...
pub(crate) async fn try_parse_api_error(response: reqwest::Response) -> Result<reqwest::Response> {
    ...
    match status.as_u16() {
        ...
        502 => return Err(Error::bad_gateway(retry_after.or(Some(30)))), // Default 30s retry
        
        // Service Unavailable - always transient
        503 => return Err(Error::service_unavailable(retry_after.or(Some(60)))), // Default 60s retry
        
        // Gateway Timeout - always transient
        504 => return Err(Error::gateway_timeout(retry_after.or(Some(45)))), // Default 45s retry
        
        // Rate Limited - always recoverable
        429 => return Err(Error::rate_limited(retry_after.or(Some(60)), None)), // Default 60s retry
        ...
        // Server errors (500-599, except those handled above)
        500..=599 => {
            // Try to parse API error response first
            let bytes = response.bytes().await.map_err(Error::Http)?;
            
            // Check if it's a structured API error
            if let Ok(api_error) = serde_json::from_slice::<ApiError>(&bytes) {
                // Check for specific server error types that suggest retry
                let message = &api_error.error.message;
                let is_retryable = message.to_lowercase().contains("server")
                    || message.to_lowercase().contains("internal error")
                ...
                let extracted_request_id = request_id.or_else(|| {
                    ...
                    if message.contains("request ID") {
                        ...
                        if let Some(start) = message.find("req_") {
                            let req_part = &message[start..];
                            if let Some(end) = req_part.find(|c: char| !c.is_alphanumeric() && c != '_') {
                                Some(req_part[..end].to_string())
                            } else {
                                // Take the rest of the string if no delimiter found
                                let words: Vec<&str> = req_part.split_whitespace().collect();
                                words.first().map(|s| s.trim_end_matches(&['.', ')', ',', ';', '!']).to_string())
                            }
                        } else {
                            None
                        }
        ...
        _ => {
            // For other status codes, try to parse as API error first
        }
    }
...
mod tests {
    ...
    fn test_new_error_types() {
        ...
        assert_eq!(bad_gateway.user_message(), "The service is temporarily unavailable. Please try again in a moment.");

        // Test ServerError
        let server_error = Error::server_error("Internal server error", Some("req_123".to_string()), true);
    ...
    fn test_user_messages() {
        ...
        assert_eq!(service_unavailable.user_message(), "Service is temporarily unavailable. Please try again in 30 seconds.");

        let rate_limited = Error::rate_limited(Some(60), None);
        assert_eq!(rate_limited.user_message(), "Rate limit exceeded. Please try again in 60 seconds.");
    ...
    fn test_error_classification() {
        ...
        for error in non_transient_errors {
            assert!(!error.is_transient(), "Error should not be transient: {:?}", error);
    ...
    fn test_comprehensive_error_handling() {
        // Test all major error types and their features
        
        // 1. Test HTTP status errors with retry logic
        let bad_gateway = Error::bad_gateway(Some(30));
        assert!(bad_gateway.is_recoverable());
        assert!(bad_gateway.is_transient());
        assert_eq!(bad_gateway.retry_after(), Some(30));
        assert!(bad_gateway.user_message().contains("temporarily unavailable"));

        // 2. Test server errors with request ID tracking
        ...
        // 3. Test client errors with field-specific guidance
        let client_error = Error::ClientError {
            message: "Invalid parameter".to_string(),
            status_code: 400,
            field: Some("temperature".to_string()),
            suggestion: Some("Temperature must be between 0 and 2".to_string()),
        };
        assert!(!client_error.is_recoverable());
        assert!(!client_error.is_transient());
        assert_eq!(client_error.retry_after(), None);
        assert!(client_error.user_message().contains("Temperature must be between 0 and 2"));

        // 4. Test authentication errors with actionable suggestions
        let auth_error = Error::AuthenticationFailed {
            message: "Invalid API key".to_string(),
            suggestion: "Please check your API key format and ensure it's valid".to_string(),
        };
        assert!(!auth_error.is_recoverable());
        assert!(!auth_error.is_transient());
        assert_eq!(auth_error.user_message(), "Please check your API key format and ensure it's valid");

        // 5. Test rate limiting with retry delays
        let rate_limit = Error::rate_limited(Some(120), Some("requests".to_string()));
        assert!(rate_limit.is_recoverable());
        assert!(rate_limit.is_transient());
        assert_eq!(rate_limit.retry_after(), Some(120));
        assert!(rate_limit.user_message().contains("120 seconds"));

        // 6. Test container expiration (existing functionality enhanced)
        let container_expired = Error::container_expired("Session expired", false);
        assert!(container_expired.is_recoverable());
        assert!(container_expired.is_container_expired());
        assert!(container_expired.is_transient());
        assert_eq!(container_expired.retry_after(), Some(1));

        // 7. Test stream errors (converted from other error types)
        let stream_error = Error::Stream("Streaming failed: Service temporarily unavailable (retry in 30s)".to_string());
        assert!(!stream_error.is_recoverable()); // Stream errors are not auto-recoverable
        assert!(!stream_error.is_transient());
    ...
    fn test_error_helper_methods() {
        ...
        for (error, expected_message) in errors_and_expected_messages {
            assert_eq!(error.user_message(), expected_message, "User message mismatch for error: {:?}", error);
        }
    }
    ...
    fn test_error_factory_methods() {
        ...
        let bad_gateway = Error::bad_gateway(Some(30));
        if let Error::BadGateway { retry_after, status_code } = bad_gateway {
        ...
        let service_unavailable = Error::service_unavailable(Some(60));
        if let Error::ServiceUnavailable { retry_after, retry_message } = service_unavailable {
        ...
        if let Error::RateLimited { retry_after, limit_type, .. } = rate_limited {
        ...
        if let Error::ServerError { message, request_id, retry_suggested, .. } = server_error {
